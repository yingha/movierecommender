...
import numpy as np
import pandas as pd
# you have already the data access of movies
from movierecommender.utils import movies, ratings, item_avg, final_rec, get_movie_id, get_user_item_matrix
import pickle
import os
...
def personalized_recommender_nmf(liked_items, item_rating, k=5):
    ...
    user_item_prediction = pd.DataFrame(
        predictions, columns=user_item_matrix.columns)
    recommendations = user_item_prediction[(movie_df == 2.5)].round(1).T
    recommendations.columns = ['predicted_rating']
    recommendations = recommendations['predicted_rating'].sort_values(
        ascending=False)
